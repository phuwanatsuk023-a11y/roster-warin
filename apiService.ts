...
function saveRosterSchedule(data) {
  ...
  // ลบข้อมูลเดิมทิ้งก่อน: ถ้าชายลบชาย และหญิงลบหญิง ในชีท roster_schedules
  const existingRows = [];
  if (rows.length > 1) {
    for (let i = 1; i < rows.length; i++) {
      const r = rows[i];
      ...
      if (targetGender === 'M') {
        // บันทึกชาย -> ลบเฉพาะข้อมูลเวรชาย (M) ของเดือน/ปีนี้ทิ้ง (คงข้อมูลเวรหญิง F ไว้)
        if (rYear === targetYear && rMonth === targetMonth && rGender === 'M') {
          continue; // ลบทิ้ง
        }
      } else if (targetGender === 'F') {
        // บันทึกหญิง -> ลบเฉพาะข้อมูลเวรหญิง (F) ของเดือน/ปีนี้ทิ้ง (คงข้อมูลเวรชาย M ไว้)
        if (rYear === targetYear && rMonth === targetMonth && rGender === 'F') {
          continue; // ลบทิ้ง
        }
      } else if (targetGender === 'ALL') {
        if (rYear === targetYear && rMonth === targetMonth) {
          continue; // ลบทั้งหมดของเดือนนี้
        }
      }
  ...
  const genderLabel = targetGender === 'M' ? 'ชาย' : (targetGender === 'F' ? 'หญิง' : 'ทั้งหมด');
  return {
    status: 'success',
    ...
    year: targetYear,
    gender: targetGender,
    message: 'บันทึกตารางเวร' + genderLabel + ' เดือน ' + targetMonth + '/' + (targetYear + 543) + ' เรียบร้อย (ลบข้อมูลเวร' + genderLabel + 'เดิมในชีทแล้ว)'
  };
}
  ...
  public async saveRosterSchedule(
    ...
    if (settings.appsScriptUrl) {
      try {
        ...
        const genderText = gender === 'M' ? 'เวรชาย' : (gender === 'F' ? 'เวรหญิง' : 'ทั้งหมด');
        return {
          count: items.length,
          message: res?.message || `บันทึกข้อมูลตาราง${genderText} ${items.length} รายการลง Google Sheet สำเร็จ (ลบข้อมูล${genderText}เดิมในชีทแล้ว)`,
        };
      } catch (err: any) {
    ...
    return {
      count: items.length,
      message: `บันทึกข้อมูลตาราง${genderText} ${items.length} รายการเรียบร้อย (ลบข้อมูล${genderText}เดิมแล้ว)`,
    };
  }
